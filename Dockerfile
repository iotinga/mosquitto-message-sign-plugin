...
RUN wget https://github.com/iotinga/mosquitto-oauth-plugin/archive/refs/tags/v${MESSAGE_SIGN_PLUGIN_VERSION}.tar.gz -O mosquitto-oauth-plugin-${MESSAGE_SIGN_PLUGIN_VERSION}.tar.gz
RUN tar xzvf mosquitto-oauth-plugin-${MESSAGE_SIGN_PLUGIN_VERSION}.tar.gz
RUN cd mosquitto-oauth-plugin-${MESSAGE_SIGN_PLUGIN_VERSION} \
    && mkdir build && cd build \
    && cmake .. \
    && make \
    && make install \
    && cd .. \
    && rm -rf mosquitto-oauth-plugin-${MESSAGE_SIGN_PLUGIN_VERSION} mosquitto-oauth-plugin-${MESSAGE_SIGN_PLUGIN_VERSION}.tar.gz
